...
local output_folder = '../tilesets'
local image_n = 0
local tileset_n = 0

-- tileset width in tiles
local tileset_width = 8
...
local function export_tileset(tileset)
  ...
  if #tileset > 0 then
    local spec = spr.spec
    spec.width = size.width * tileset_width
    spec.height = size.height * math.ceil(#tileset / tileset_width)
    local image = Image(spec)
    image:clear()
    local y = 0
    local x = 0

    -- draw tiles from left to right from top to bottom
    for i = 0,#tileset-1 do
      local tile = tileset:getTile(i)
      image:drawImage(tile, x, y)

      -- if of the screen return to the next line
      x = x + size.width
      if x >= spec.width then
        x = 0
        y = y + size.height
      end
    end
    ...
    -- for i = 0,#tileset-1 do
    --   local tile = tileset:getTile(i)
    --   image:drawImage(tile, i*size.width, y)

    --   -- if of the screen return to the next line
    --   if i % tileset_width == tileset_width-1 then
    --     y = y + size.height
    --   end
    -- end
